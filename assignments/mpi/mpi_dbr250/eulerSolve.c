...
{
  MPI_Status status;
  MPI_Datatype CONSERVED;//make a structure in MPI for conserveds
  struct Conserved incons;
  build_CONSERVED(incons, &CONSERVED);
  
  
  struct Conserved GhostsR[2];//Ghost cells
  struct Conserved GhostsL[2];
  struct Conserved IR[2];//Interior cells
  struct Conserved IL[2];

  //Do boundary condition, three cases : the left most, rightmost and interior processors
			...
			double BCnum, int my_rank, int np)
{
  ...
  //Makes sure all lamdamax is for all the cell
...
int main(int argc, char **argv)
{
  ...
  //Making the array of x values. It depends on which processor we are in.
  double x0, x1;
  int N;
  double dx2ndGlobal;
  ...
  if(InitialType == 1)
    {
      ...
      //processor zero writes first and then the other procs append
      if(my_rank == 0)
	{
  ...
  else if(InitialType == 2)//Type 2-4 are not supported yet
    {
