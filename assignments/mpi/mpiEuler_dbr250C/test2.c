...
int main(int argc, char **argv)
{
  ...
  if(my_rank == 0)
    {
      MPI_Send(&inconsArray, 3, CONSERVED, 1, tag,  MPI_COMM_WORLD);
    }
  if(my_rank != 0 && my_rank != p-1)
    {
      MPI_Recv(&inconsArray, 3, CONSERVED, my_rank-1,
	       tag,  MPI_COMM_WORLD, &status);
      MPI_Send(&inconsArray, 3, CONSERVED, my_rank+1, 
	       tag,  MPI_COMM_WORLD);
    }
  else
    {
      MPI_Recv(&inconsArray, 3, CONSERVED, my_rank-1,
	       tag,  MPI_COMM_WORLD, &status);
      MPI_Send(&inconsArray, 3, CONSERVED, 0, 
	       tag,  MPI_COMM_WORLD);
    }
  printf("This is inconsArray[1] on processor %d:  %f %f %f \n", my_rank, 
	 inconsArray[1].rho,
