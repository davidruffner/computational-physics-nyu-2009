			...
			double BCnum)
{
  ...
  //printf("BCs %f \n", dt);
  //printf("LU[0].px = %f, LU[1].px = %f \n", LU[0].px, LU[1].px);
  
  struct Conserved *Utemp = (struct Conserved*) calloc(N, 
			    sizeof(struct Conserved));
...
int main()
{
  ...
  write_ascii_tableCons("testInitial2nd.cfg",x2nd, Uinitial2nd ,Nx+4);
  ...
  */
  ...
  while(time<= runTime)
    {
      j++;
      time = time + evolveF_HLL_1stO(Uinitial, Nx, dx, CFL, BCnum);
    }
   ...
   while(time<= runTime)
    {
      j++;
      time = time + evolveF_HLL_2ndO(Uinitial2nd, Nx, dx2nd, CFL, BCnum);
    }
   ...
   write_ascii_tableCons("testFinalCons2nd.cfg",x2nd, Uinitial2nd,Nx+4);
   write_ascii_tablePrim("testFinalPrim2nd.cfg",x2nd, Pinitial2nd,Nx+4);
